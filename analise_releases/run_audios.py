"""
Runner para baixar audios de teleconferencias de resultados de todas as empresas.

Uso:
    python run_audios.py              # Todas as empresas
    python run_audios.py cury mrv     # Empresas especificas
"""
...
def rodar_empresa(chave):
    if chave == "tenda":
        scraper = TendaRI(tipo_documento="audio")
        resultado = scraper.executar()
        return ("Tenda", resultado)
    else:
        config = EMPRESAS_MZ[chave]
        scraper = MZGroupRI(config, tipo_documento="audio")
        resultado = scraper.executar()
...
def main():
    ...
    print("=" * 60)
    print("DOWNLOAD DE AUDIOS DE TELECONFERENCIAS")
